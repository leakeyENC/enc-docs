        ...
        <div style={{ background: "var(--c-surface)", minHeight: "calc(100vh - 64px)" }}>
          <div style={{ maxWidth: "720px", margin: "0 auto", padding: "38px 44px 90px" }}>
            ...
            <p style={{ fontSize: "14.5px", color: "var(--c-text-muted)", margin: "0 0 16px" }}>
              Content-Type: <code style={inlineCode}>application/json</code> · Fields marked <span style={{ color: "#E5484D", fontWeight: 700 }}>*</span> are required.
            </p>
            <div style={{ border: "1px solid var(--c-border-card)", borderRadius: "12px", overflow: "hidden", marginBottom: "20px" }}>
              ...
              {ONBOARDING_FIELDS.map((f, i) => (
                <div key={f.name} style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 2.3fr", borderTop: "1px solid var(--c-border-faint)", fontSize: "13.5px", background: rowBg(i) }}>
                  <div style={{ padding: "12px 15px", fontFamily: MONO, fontSize: "12.5px", color: "var(--c-table-name)", fontWeight: 500, wordBreak: "break-word" }}>{f.name}<span style={{ color: "#E5484D", fontWeight: 700 }}>{f.req}</span></div>
            ...
            <div style={{ border: "1px solid var(--c-border-card)", borderRadius: "12px", overflow: "hidden", marginBottom: "34px", background: "var(--c-surface-sunken)" }}>
              ...
              <pre className="ed-scroll" style={{ margin: 0, overflow: "auto", padding: "15px 18px", fontFamily: MONO, fontSize: "12.5px", lineHeight: 1.65, color: "var(--c-code-text)", whiteSpace: "pre" }}>{REQUEST_JSON}</pre>
            </div>
            ...
            <div style={{ display: "flex", gap: "8px", marginBottom: "16px", flexWrap: "wrap" }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: "7px", fontFamily: MONO, fontSize: "13px", fontWeight: 600, padding: "6px 13px", borderRadius: "8px", background: "var(--c-success-bg)", color: "var(--c-success-text)" }}>
                <span style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#1F9D63" }} />201 Created
              </span>
              {chip("400")}{chip("401")}{chip("409")}
            </div>
            <div style={{ border: "1px solid var(--c-border-card)", borderRadius: "12px", overflow: "hidden", marginBottom: "20px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 2.3fr", ...headRow }}>
              ...
              {RESP_FIELDS.map((f, i) => (
                <div key={f.name} style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 2.3fr", borderTop: "1px solid var(--c-border-faint)", fontSize: "13.5px", background: rowBg(i) }}>
                  <div style={{ padding: "12px 15px", fontFamily: MONO, fontSize: "12.5px", color: "var(--c-table-name)", fontWeight: 500, wordBreak: "break-word" }}>{f.name}</div>
                  <div style={{ padding: "12px 15px", color: "var(--c-type)", fontFamily: MONO, fontSize: "12px" }}>{f.type}</div>
                  <div style={{ padding: "12px 15px", color: "var(--c-text-secondary)", lineHeight: 1.55 }}>{f.desc}</div>
                </div>
              ))}
            ...
            <div style={{ border: "1px solid var(--c-border-card)", borderRadius: "12px", overflow: "hidden", marginBottom: "34px", background: "var(--c-surface-sunken)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "9px 15px", borderBottom: "1px solid var(--c-border-card)" }}>
                <span style={{ fontFamily: MONO, fontSize: "11px", letterSpacing: ".08em", textTransform: "uppercase", color: "var(--c-text-faint)", fontWeight: 600 }}>Example response · 201 · JSON</span>
              </div>
              <pre className="ed-scroll" style={{ margin: 0, overflow: "auto", padding: "15px 18px", fontFamily: MONO, fontSize: "12.5px", lineHeight: 1.65, color: "var(--c-code-text)", whiteSpace: "pre" }}>{RESPONSE_JSON}</pre>
            </div>
